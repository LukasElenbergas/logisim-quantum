...
class PauliXGate extends AbstractQuantumGate {
    ...
    private PauliXGate() {
        super("Pauli-X Gate",
                Strings.getter("paulixGateComponent"),
                Icons.getIcon("xGate.gif"),
                "X");
    }
    ...
    public void propagate(InstanceState state) {
        ...
        if (in.isQuantum() && top != Value.ERROR) {
            ...
            if (top == Value.UNKNOWN) {
                Value out = new Value(Value.QUANTUM, new QuantumValue(in.qVal, "X"));

                state.setPort(0, out, 1);

            } else if (top.isQuantum()) {

                if (top.qVal.instructions.getLast().equals("C")){
                    Value out = new Value(Value.QUANTUM, new QuantumValue(in.qVal, "CX"));
                    state.setPort(0, out, 1);
                } else if (top.qVal.instructions.getLast().equals("CC")) {
                    Value out = new Value(Value.QUANTUM, new QuantumValue(in.qVal, "CCX"));
                    state.setPort(0, out, 1);
                }
            }
        } else {
